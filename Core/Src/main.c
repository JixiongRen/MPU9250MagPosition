...
//   // 检查 AK8963 是否正常
//   if (Mag_WHOAMI(mpu)) {
//       char msg[] = "AK8963 init success!\r\n";
//       HAL_UART_Transmit(&huart1, (uint8_t *)msg, strlen(msg), 1000);
//   } else {
//     char msg[] = "AK8963 init failed!\r\n";
//     HAL_UART_Transmit(&huart1, (uint8_t *)msg, strlen(msg), 1000);
//   }
//   /* USER CODE END 2 */
//
...
//     /* USER CODE BEGIN 3 */
//     // 读取磁力计数据
//     READ_MPU9250_MAG(mpu);
//     char buffer[50];
//     snprintf(buffer, sizeof(buffer), "Mag (X,Y,Z): %d,%d,%d\r\n", mpu_value.Mag[0], mpu_value.Mag[1], mpu_value.Mag[2]);
//
//     // 通过串口发送数据
...
//
//   /*Configure GPIO pin : MPU9250_CHIP_SELECT_Pin */
//   GPIO_InitStruct.Pin = MPU9250_CHIP_SELECT_Pin;
//   GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
//   GPIO_InitStruct.Pull = GPIO_NOPULL;
//   GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
//   HAL_GPIO_Init(MPU9250_CHIP_SELECT_GPIO_Port, &GPIO_InitStruct);
//
...
int main(void)
{
  ...
  MX_SPI2_Init();
  /* USER CODE BEGIN 2 */
  mpu.mpu9250_cfg.hspix = hspi1;
  mpu.mpu9250_cfg.CS_Port = GPIOA;
  mpu.mpu9250_cfg.CS_Pin = GPIO_PIN_11;
  MPU9250_Init(mpu);
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    char buffer[50];
    snprintf(buffer, sizeof(buffer), "MX:%.3f \tMY:%.3f\t MZ:%.3f\r\n", mpu.mpu_value.Mag[0], mpu.mpu_value.Mag[1], mpu.mpu_value.Mag[2]);
    HAL_UART_Transmit(&huart1, (uint8_t *)buffer, strlen(buffer), HAL_MAX_DELAY);
    HAL_Delay(1);
